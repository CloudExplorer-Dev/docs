...
!!! Abstract ""
    ...
    按照部署服务器要求准备好部署环境后，可通过CloudExplorer-Lite快速安装脚本一键快速部署。  
...
## 3 部署完成  
!!! Abstract ""   
    CloudExplorer-Lite是一款B/S架构的产品，即浏览器/服务器结构，在服务器安装完成后，客户端通过浏览器访问以下地址，即可开始使用。
    ```
    http://目标服务器IP地址：服务运行端口
    默认用户名：admin，默认密码：cloudexplorer-lite
    ```