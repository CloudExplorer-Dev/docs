!!! Abstract ""
    ...
    当用户需要新增云主机资源时，无需提前操作任何配置，可以直接在云主机列表上方点击【创建】,创建云主机流程如下图：

![云主机创建流程](../../img/cloud-server/create/云主机创建流程.png){ width="1235px" }

!!! Abstract ""

    在【虚拟机服务】-【云主机】页面，点击【创建】即可选择云账号，创建云主机。
...
!!! Abstract ""
    ...
    用户需对系统盘设置磁盘类型，并定义磁盘大小，及是否随实例删除。数据盘可添加多块，并分别对每块设置磁盘类型、磁盘大小、是否随实例删除。

!!! Abstract ""

    用户可通过调整购买数量设置一次创建多台虚拟机。<br />
    创建包年包月的云主机时,还需要设置购买时长。<br />
    购买数量为多台时，每台云主机的实例规格相同。若用户想要申请不同实例规格的多台云主机，则需要分多次申请。

![阿里基础配置](../../img/cloud-server/create/阿里云基础配置.png){ width="1235px" }
...
![阿里网络配置](../../img/cloud-server/create/阿里云网络配置.png){ width="1235px" }
...
![阿里系统配置](../../img/cloud-server/create/阿里云系统配置.png){ width="1235px" }
...
![阿里确认信息](../../img/cloud-server/create/阿里云确认信息.png){ width="1235px" }
...
!!! Abstract ""
    ...
    基础配置页面需要用户确认云主机付费方式，选择云主机所在区域和可用区，设置云主机实例规格大小，选择操作系统和版本，并为云主机配置磁盘。<br />
    创建包年包月的云主机时,还需要设置购买时长。<br />
    用户可对系统盘设置磁盘类型，并增大磁盘大小，华为云云主机系统盘默认为随实例删除，用户不可修改。数据盘可添加多块，并分别对每块设置磁盘类型、磁盘大小、是否随实例删除。
...
![腾讯基础配置](../../img/cloud-server/create/腾讯云基础配置.png){ width="1235px" }
...
![腾讯网络配置](../../img/cloud-server/create/腾讯云网络配置.png){ width="1235px" }
...
![腾讯系统配置](../../img/cloud-server/create/腾讯云系统配置.png){ width="1235px" }
...
![腾讯信息确认](../../img/cloud-server/create/腾讯云信息确认.png){ width="1235px" }
...
!!! Abstract ""
    ...
    基础配置页面需要用户设置云主机所属的数据中心、集群，选择需要使用的镜像模板，输入实例规格大小，可添加多块数据盘并设置每块数据盘大小，选择磁盘是否随实例删除。

!!! Abstract ""

    云主机的系统盘大小根据用户所选择的镜像模板中的系统盘大小确定，不可编辑修改但可以选择是否随实例删除。如用户所选镜像模板中带有数据盘，则此块数据盘大小也不可编辑修改，但可以选择是否随实例删除。
...
!!! Abstract ""
    ...
    - Automatic selection：只有在基础配置中所选的集群开启了 DRDS 功能时，才会显示该类型，选择该类型后，会根据 VMware 的均衡资源策略分配云主机资源。

!!! Abstract ""

    存储资源需要选择创建云主机所需的存储资源。
...
!!! Abstract ""
    ...
    如需一次创建多台云主机，则系统配置页面会分多个标签页设置每台云主机的名称和 Hostname。<br />
...
!!! Abstract ""
    ...
    基础配置页面需要用户设置云主机所在区域，选择需要使用的镜像模板、实例规格大小，及是否创建启动盘。可添加多块数据盘并设置每块数据盘的磁盘类型和磁盘大小。

!!! Abstract ""

    云主机的系统盘大小根据用户所选择的实例规格中硬盘大小确定，但用户可以调整磁盘大小，并需要选择磁盘类型。
