...
!!! Abstract ""
    进入CloudExplorer-Lite首页后：
    导航栏：
    左侧导航是固定模块菜单，有【首页】、【管理中心】、【云主机服务】、【云账单】、【运营分析】、【安全合规】六大模块，点击图标便可切换模块中。

    工作区域：
    右侧区域为整体预览系统的功能及管理资源的数据统计，主要分为用户快捷服务、管理资源统计、费用趋势、资源优化建议、安全合规扫描结果等；  
    快捷服务：用户可以点击快捷服务的按钮或菜单，可以快速进入对应的功能页面。
    数据统计：统计用户当前管理的资源数据、费用数据以及需要资源优化的数据和不合规的资源数据。
...
!!! Abstract ""
    管理员在此模块可以纳管云账号和同步资源数据、企业组织架构和用户权限的管理，是后续各模块查看、管理和分析资源的来源，具体可参考本文档的功能手册【管理中心】。
...
!!! Abstract ""
    用户可在此模块对云主机创建、启停机、配置变更等，云磁盘的创建、挂载/卸载、扩容等全生命周期的运维管理操作；管理员除运维管理操作外还可以在此模块进行资源授权，具体可参考本文档的功能手册【云主机服务】。

![云主机服务](img/quick_start/云主机服务.png){ width="1235px" }

- 云账单服务  

!!! Abstract ""
    用户可在此模块查看 IT 资源投入的成本费用、管理员可在此模块设置费用分摊、自定义多维度费用分析报表，具体可参考本文档的功能手册【云账单】。

![云账单](img/quick_start/云账单.png){ width="1235px" }
...
![运营分析](img/quick_start/运营分析.png){ width="1235px" }
...
![安全合规](img/quick_start/安全合规.png){ width="1235px" }
...
!!! Abstract ""
    本章主要介绍系统管理员如何使用CloudExplorer-Lite 快速管理一个云账号，帮助大家快速掌握使用方法。  
    初始化配置步骤：创建云账号 -> 管理云主机 -> 查看云账单 -> 查看运营分析 -> 查看安全合规；  

### 3.2 创建云账号

!!! Abstract ""
    打开【管理中心】——【云账号】菜单，创建一个阿里云的云账号，名称为“阿里云-产品”。

![云账号列表](img/quick_start/云账号列表.png){ width="1235px" }

!!! Abstract ""
    云平台选择“阿里云”(若是其他云平台的可选择对应的云平台)，当前以“阿里云”为例。

![添加云账号](img/quick_start/添加云账号.png)

!!! Abstract ""
    输入名称为“阿里云-产品”，输入云账号的 AccessKey ID、AccessKey Secret，点击“确认”。
    说明：AccessKey权限要求：ECS的管理权限和所有资源的只读权限。

![输入AccessKey](img/quick_start/输入AccessKey.png)

!!! Abstract ""
    系统会自动同步该云账号下的云主机、磁盘、镜像等资源数据，过去一年的账单数据和安全合规扫描的资源数据。
    系统会定时同步数据，用户也可以手动同步，详情操作可
    资源同步完成后便可使用各服务模块进行查看和管理，否则各服务模块将没有数据。

![同步数据](img/quick_start/同步全部数据.png)

### 3.3 管理云主机

!!! Abstract ""
    【云主机服务】模块提供了云主机的创建、启/停机、回收、配置变更、扩容、授权等。同时基于用户管理体系，用户可创建、可操作的资源都具备隔离性。在多云环境下提供统一的操作界面，为用户带来极大便利。
...
![选择云账号](img/quick_start/创建阿里云主机-选择云账号.png)
...
![创建中云主机列表](img/quick_start/创建中云主机列表.png)
![创建云主机任务](img/quick_start/创建云主机任务.png)
!!! Abstract ""
    任务完成后，云主机状态变为“运行中”。
...
!!! Abstract ""
    云账单服务实现了公有云费用汇总、费用分摊、自定义多维度月账单功能，在多云多账号的环境下提供统一的查看费用界面，方便用户管理费用。

- 账单总览

!!! Abstract ""
    系统管理员可以看到所有纳管云账号的当月总花费、今年总花费和近半年/近一年的总费用趋势，下方的月账单中可以查看用户自定义不同维度的月账单。
    组织管理员和普通用户可以查看已被分账授权的当月总花费、今年总花费和近半年/近一年的总费用趋势，下方的月账单中可以查看已被分账授权的不同维度的月账单。

![云账单](img/quick_start/云账单.png)
...
!!! Abstract ""
    ...
    若想要展示其他维度的月账单，系统管理员添加自定义月账单，支持的统计维度字段：云平台、云账号、区域、企业项目、组织（仅一级组织）、组织层级树、工作空间、标签、产品名称、产品明细、付费方式、资源名称等。

![自定义账单](img/quick_start/自定义账单.png)

!!! Abstract ""
    创建一个“按工作空间”的自定义账单，维度是：组织层级树1级、组织层级树2级、工作空间、产品名称。

![添加自定义账单](img/quick_start/添加自定义账单.png)

!!! Abstract ""
    自定义账单创建完成后可在【账单总览】中下面的月账单中查看。

![账单总览](img/quick_start/账单总览.png)
...
![安全合规总览](img/quick_start/安全合规总览.png)
...
![合规规则设置](img/quick_start/合规规则设置.png)

!!! Abstract ""
    新增一个阿里云的磁盘是否开启加密设置的检测规则，操作如下：

![添加合规规则](img/quick_start/添加合规规则.png)

!!! Abstract ""
    添加完规则后，在【合规扫描】中变可以看到该规则，点击【一键扫描】后可以对该规则进行资源扫描。
![合规规则扫描](img/quick_start/合规规则扫描.png)
...
![合规规则组](img/quick_start/合规规则组.png)
...
### 3.6 查看运营分析
!!! Abstract ""
    添加完云账号后便可对已纳管的资源从分布、容量、使用情况等进行各维度的分析，提供资源优化建议。
...
![运营分析总览](img/quick_start/运营分析.png)
...
![云磁盘优化](img/quick_start/云磁盘分析.png)

- 云主机优化

    根据优化策略查询条件可以查找出需要优化的资源；
    支持查询升/降配云主机、变更付费方式云主机、建议回收的云主机；
    用户可以自定义优化策略的判断条件；

![云主机优化](img/quick_start/资源优化.png)

!!! Abstract ""
    点击每个优化策略面板右上角的设置按钮，可以对优化策略条件进行自定义设置，设置应用后则按照最新的策略进行查询资源。

![资源优化策略](img/quick_start/资源优化策略.png)
